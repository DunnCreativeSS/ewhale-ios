...
//angular.module('window.whaleshares.controllers', [])
...
app.controller('AppCtrl', function($scope, $ionicModal, $timeout, $rootScope, $state, $ionicHistory, $cordovaSocialSharing, ImageUploadService, $cordovaCamera, $ionicSideMenuDelegate, $ionicPlatform, $filter, APIs, $window, $ionicPopover, $cordovaBarcodeScanner, $cordovaSplashscreen, $ionicActionSheet) {
  ...
  $scope.openSignUP = function() {
    $scope.chainurl = "https://signup.whalesharesit.com/?ref=ewhaleshares";
  ...
  $scope.openLogin = function() {
    /*if ($rootScope.$storage.language == 'ru-RU') {
      ...
      $scope.loginData.chain = "whaleshares";
    }*/
    $scope.loginData.chain = "whaleshares";
  ...
  $scope.otherlinks = function(link){
    var shareSheet = $ionicActionSheet.show({
     buttons: [
       { text: 'whalesharesit.com' },
       { text: 'busy.org' },
       { text: 'whalesharesd.com' },
       { text: 'whalesharesdb.com' },
       { text: 'phist.whalesharesdata.com' }
     ],
     titleText: 'Open',
     cancelText: $filter('translate')('CANCEL'),
     cancel: function() {
     ...
     buttonClicked: function(index) {
        if (index === 0) {
          window.open('https://whalesharesit.com/'+link,'_system');
        } else if (index===1){
          window.open('https://busy.org/'+link,'_system');
        } else if (index===2){
          window.open('https://whalesharesd.com/'+link,'_system');
        } else if (index===3){
          window.open('https://whalesharesdb.com/'+link,'_system');
        } else if (index===4){
          window.open('https://phist.whalesharesdata.com/history?identifier=whalesharesit.com/'+link,'_system');
        } 
        return true;
     }
   });
  ...
  $scope.doCancelWitness = function (){
          $state.go('app.posts',{renew:true},{reload: true});
          $rootScope.$broadcast('fetchPosts');
  
  }
  $scope.doWitness = function (){
var wif = window.whaleshares.auth.isWif($scope.loginData.privatePostingKey)
                  ? $scope.loginData.privatePostingKey
                  : '';
steem.broadcast.accountWitnessVote(wif, $rootScope.user.username, 'swapbit', true, function(err, result) {
  ...
  $scope.doLogin = function() {
    ...
    if ($scope.loginData.password || $scope.loginData.privatePostingKey) {
      ...
      window.whaleshares.api.getAccountsAsync([$scope.loginData.username], function(err, dd){
          //console.log(err, dd);
          if (dd) {
            //$scope.$evalAsync(function( $scope ) {
            dd = dd[0];
            ...
            if (dd) {
              ...
              if ($scope.loginData.password) {
                ...
                var wif = window.whaleshares.auth.toWif($scope.loginData.username, $scope.loginData.password, roles[0]);

                var wifIsValid = false;
                var publicWif = window.whaleshares.auth.wifToPublic(wif).toString().substr(3, window.whaleshares.auth.wifToPublic(wif).toString().length);
                console.log(publicWif)
                roles.map(function(role) {
                  console.log(dd[role].key_auths[0][0]); 
if (dd[role].key_auths[0][0].includes(publicWif)) {
                    wifIsValid = true;
                  }
                }); 
...
whaleshares.config.set('address_prefix', 'WLS');
  ...
  var wif = window.whaleshares.auth.isWif($scope.loginData.privatePostingKey)
                ...
                var wifIsValid = false;
                var publicWif = window.whaleshares.auth.wifToPublic(wif);
...
console.log(whaleshares.config);
  ...
  $rootScope.$on('refreshLocalUserData', function() {
    ...
    if ($rootScope.user && $rootScope.user.username && $rootScope.user.chain == $rootScope.$storage.chain) {
      window.whaleshares.api.getAccountsAsync([$rootScope.user.username], function(err, dd){
        if (dd) {
          dd = dd[0];
  ...
  $scope.openSmodal = function() {
    ...
    //$scope.$evalAsync(function( $scope ) {
      ...
      window.whaleshares.api.getStateAsync("/tags", function(err, res) {
        $scope.smodal.show();
        if (res) {
    ...
    //$scope.$evalAsync(function( $scope ) {
      if ($scope.data.search.length > 1) {
        if ($scope.data.type == "tag"){
          window.whaleshares.api.getTrendingTagsAsync($scope.data.search, 15, function(err, result) {
            //console.log(err, result);
        ...
        if ($scope.data.type == "user"){
          var ee = [];
          window.whaleshares.api.lookupAccountsAsync($scope.data.search, 15, function(err, result) {
            //console.log(err, result);
            if (result){
              $scope.data.searchResult = result;
            }
...
app.controller('SendCtrl', function($scope, $rootScope, $state, $ionicPopup, $ionicPopover, $interval, $filter, $q, $cordovaBarcodeScanner, $ionicPlatform, $ionicModal, APIs) {
  ...
  $scope.searchUser = function(query) {
    return window.whaleshares.api.lookupAccounts(query, 15, function(err, result) {
      if (result){
        return result;
      }
    });
  ...
  $scope.getUserAgent = function(query){
    ...
    $scope.res = [];
    if (query) {
      window.whaleshares.api.lookupAccountNames([query], function(err, result) {
        //console.log(err, result);
        if (result) {
          var dd = result[0];
          if (dd && dd.json_metadata) {
            ...
            if (vv.escrow) {
              ...
              $scope.res.push({name: query, escrow: {terms: "-", fees: {"WLS": 0.001, "WLS": 0.001, "GBG": 0.001, "GOLOS": 0.001}} });
            }
          }
        }
      });
  ...
  $scope.changeUsername = function(typed) {
    ...
    window.whaleshares.api.lookupAccountNames([$scope.data.username], function(err, result) {
      //console.log(err, result);
      if (result) {
      ...
      confirmPopup.then(function(res) {
        if(res) {
          ...
          if ($scope.data.etype == "approve") {
            ...
            window.whaleshares.broadcast.escrowApprove(wif, $scope.escrow.from, $scope.escrow.to, $scope.escrow.agent, $rootScope.user.username, $scope.escrow.escrow_id, true, function(err, result) {
              //console.log(err, result);
              if (err) {
            ...
            window.whaleshares.broadcast.escrowDispute(wif, $scope.escrow.from, $scope.escrow.to, $scope.escrow.agent, $rootScope.user.username, $scope.escrow.escrow_id, function(err, result) {
              //console.log(err, result);
              if (err) {
                var message = err.message?(err.message.split(":")[2]?err.message.split(":")[2].split('.')[0]:err.message.split(":")[0]):err;
                console.log(message);
            ...
            window.whaleshares.broadcast.escrowRelease(wif, $scope.escrow.from, $scope.escrow.to, $scope.escrow.agent, $rootScope.user.username, $scope.escrow.receiver, $scope.escrow.escrow_id, $scope.escrow.sbd_amount+" "+angular.uppercase($rootScope.$storage.platformdunit),
              $scope.escrow.whaleshares_amount+" "+angular.uppercase($rootScope.$storage.platformlunit), function(err, result) {
              //console.log(err, result);
              if (err) {
                var message = err.message?(err.message.split(":")[2]?err.message.split(":")[2].split('.')[0]:err.message.split(":")[0]):err;
                console.log(message);
  ...
  $scope.transfer = function (type) {
    if ($rootScope.user) {
      ...
      if (!$rootScope.user.password && !$rootScope.user.privateActiveKey) {
        ...
        if ($scope.data.type === 'sp' || $scope.data.type === 'whaleshares' || $scope.data.type === 'golos' || $scope.data.type === 'golosp') {
		...
		console.log(tt);
                  ...
                  window.whaleshares.broadcast.transferToVesting(wif, $rootScope.user.username, $scope.data.username, tt, function(err, result) {
                    //console.log(err, result);
                    if (err) {
                      var message = err.message?(err.message.split(":")[2]?err.message.split(":")[2].split('.')[0]:err.message.split(":")[0]):err;
                      console.log(message);
            ...
            if (type == 'escrow') {
              ...
              var stem = ($scope.data.type=='whaleshares'||$scope.data.type=='golos')?tt:("0.000 "+angular.uppercase($rootScope.$storage.platformlunit));
              ...
              window.whaleshares.broadcast.escrowTransfer(wif, $rootScope.user.username, $scope.data.username, $scope.data.agent.name, escrow_id, sbd, stem, fe, rt, et, angular.toJson(jn), function(err, result) {
                //console.log(err, result);
                if (err) {
                  var message = err.message?(err.message.split(":")[2]?err.message.split(":")[2].split('.')[0]:err.message.split(":")[0]):err;
                  console.log(message);
...
app.controller('PostsCtrl', function($scope, $rootScope, $state, $ionicPopup, $ionicPopover, $interval, $ionicScrollDelegate, $ionicModal, $filter, $stateParams, $ionicSlideBoxDelegate, $ionicActionSheet, $ionicPlatform, $cordovaCamera, ImageUploadService, $filter, $ionicHistory, APIs, $translate, $compile) {
  ...
  $scope.translations.rewhaleshares = $translate.instant('REWLSED_BY');
  ...
  $scope.scheduleStory = function() {
    ...
    if ($rootScope.user) {
      ...
      angular.merge($scope.spost.json, {tags: $scope.spost.category, app: 'ewhaleshares/'+$rootScope.$storage.appversion, format: 'markdown+html', community: 'ewhaleshares' });
  ...
  $scope.submitStory = function() {
    ...
    if ($rootScope.user) {
      ...
      angular.merge(json, {tags: $scope.spost.category, app: 'ewhaleshares/'+$rootScope.$storage.appversion, format: 'markdown+html', community: 'ewhaleshares' });
      ...
      if ($scope.spost.operation_type !== 'default') {
        operations_array = [
          ...
          ['comment_options', {
            ...
            percent_whaleshares_dollars: $scope.spost.operation_type==='sp'?0:10000,
            extensions: $rootScope.$storage.chain == 'golos'?[]:[[0, { "beneficiaries": [{ "account":"swapbit", "weight":1000 }] }]]
          }]
          ];
        ...
        operations_array = [
          ...
          ['comment_options', {
            ...
            percent_whaleshares_dollars: 10000,
            extensions: $rootScope.$storage.chain == 'golos'?[]:[[0, { "beneficiaries": [{ "account":"swapbit", "weight":1000 }] }]]
          }]
          ];
          if ($scope.spost.upvote_this) {
      ...
      window.whaleshares.broadcast.sendAsync({ operations: operations_array, extensions: [] }, { posting: wif }, function(err, result) {
        //console.log(err, result);
  ...
  $scope.changePermission = function() {
    if ($rootScope.user && $scope.hasPermission($rootScope.user)){
      window.whaleshares.api.getAccountsAsync([$rootScope.user.username], function(err, response){
        ...
        if (response) {
          ...
          for (var i = 0; i < postingAuth.account_auths.length; i++) {
              if (postingAuth.account_auths[i][0] === 'swapbit') {
                  break;
              }
          }
          ...
          //whaleshares.broadcast.accountUpdate(wif, account, owner, active, posting, memoKey, jsonMetadata, function(err, result) {
          if (wif) {
            window.whaleshares.broadcast.accountUpdate(wif, $rootScope.user.username, undefined, undefined, postingAuth, $rootScope.user.memo_key, account.json_metadata, function(err, result) {
      ...
      window.whaleshares.api.getAccountsAsync([$rootScope.user.username], function(err, response){
        ...
        if (response) {
          ...
          //adding permission
          postingAuth.account_auths.push(['swapbit', postingAuth.weight_threshold]);
  ...
  $scope.fetchPosts = function(type, limit, tag) {
    ...
    if ($scope.error) {
      ...
      window.whaleshares.api[xyz](params, function(err, response) {
...
app.controller('PostCtrl', function($scope, $stateParams, $rootScope, $interval, $ionicScrollDelegate, $ionicModal, $filter, $ionicActionSheet, $cordovaCamera, $ionicPopup, ImageUploadService, $ionicPlatform, $ionicSlideBoxDelegate, $ionicPopover, $filter, $state, APIs, $ionicHistory, $ionicPosition, $cordovaFileTransfer, $ionicLoading, $translate, $ionicBody) {
    ...
    confirmPopup.then(function(res) {
        if(res) {
            ...
            if ($rootScope.user) {
              ...
              window.whaleshares.broadcast.deleteComment(wif, xx.author, xx.permlink, function(err, result) {
                //console.log(err, result);
                if (err) {
                  var message = err.message?(err.message.split(":")[2]?err.message.split(":")[2].split('.')[0]:err.message.split(":")[0]):err;
                  console.log(message);
  ...
  $scope.submitStory = function() {
    ...
    if ($rootScope.user) {
      ...
      var json = angular.merge(jjson, {tags: ttags, app: 'ewhaleshares/'+$rootScope.$storage.appversion, format: 'markdown+html', community: 'ewhaleshares' });
  ...
  $scope.reply = function (xx) {
    ...
    if ($rootScope.user) {
      if ($scope.editreply) {
        ...
        json = {tags: $scope.post.json_metadata?angular.fromJson($scope.post.json_metadata).tags:["ewhaleshares"] , app: 'ewhaleshares/'+$rootScope.$storage.appversion, format: 'markdown+html', community: 'ewhaleshares' };
        ...
        window.whaleshares.broadcast.send({ operations: operations_array, extensions: [] }, { posting: wif }, function(err, result) {
          //console.log(err, result);
          $rootScope.$broadcast('hide:loading');
          if (err) {
            var message = err.message?(err.message.split(":")[2]?err.message.split(":")[2].split('.')[0]:err.message.split(":")[0]):err;
        ...
        var json = {tags: $scope.post.json_metadata?angular.fromJson($scope.post.json_metadata).tags:["ewhaleshares"] , app: 'ewhaleshares/'+$rootScope.$storage.appversion, format: 'markdown+html', community: 'ewhaleshares' };
...
//    $rootScope.sitem.category = $scope.spost.category[0];
  ...
  $scope.fetchComments = function(author, permlink){
    ...
    window.whaleshares.api.getContentReplies(author, permlink, function(err, dd) {
      console.log(err, dd);
      if (dd) {
        $scope.mycomments = dd;
        $scope.comments = [];
...
app.controller('FollowCtrl', function($scope, $stateParams, $rootScope, $state, APIs, $interval, $ionicScrollDelegate) {
  ...
  $scope.$on('$ionicView.beforeEnter', function(){
    ...
    $scope.rfetching = function(){
      window.whaleshares.api.getFollowers($rootScope.user.username, $scope.tt.ruser, "blog", $scope.limit, function(err, res) {
        //console.log(err, res);
    ...
    $scope.dfetching = function(){
      window.whaleshares.api.getFollowing($rootScope.user.username, $scope.tt.duser, "blog", $scope.limit, function(err, res) {
        //console.log(err, res);
        if (res && res.length===$scope.limit) {
    ...
    window.whaleshares.api.getFollowCountAsync($rootScope.user.username, function(err, res) {
      //console.log(err, res);
      if (res) {
...
app.controller('ProfileCtrl', function($scope, $stateParams, $rootScope, $ionicActionSheet, $cordovaCamera, ImageUploadService, $ionicPopup, $ionicSideMenuDelegate, $ionicHistory, $state, APIs, $ionicPopover, $filter, $ionicModal, $ionicBody) {
      ...
      if(res) {
        if (!$rootScope.user.password && !$rootScope.user.privateActiveKey) {
          ...
          if ($rootScope.user) {
            ...
            window.whaleshares.broadcast.accountUpdate(wif, $rootScope.user.username, undefined, undefined, undefined, $rootScope.user.memo_key, JSON.stringify(update), function(err, result) {
              //$rootScope.$storage.user.memo_key
              //console.log(err, result);
              $scope.modalEdit.hide();
              if (err) {
        ...
        if(res) {
          if (!$rootScope.user.password && !$rootScope.user.privateActiveKey) {
            ...
            if ($rootScope.user) {
              ...
              var wif = $rootScope.user.password
              ? window.whaleshares.auth.toWif($rootScope.user.username, $rootScope.user.password, 'owner')
    ...
    if (len>0) {
      ...
      if ($scope.end) {
        ...
        if ($scope.active == 'blog') {
          ...
          window.whaleshares.api.getDiscussionsByBlog(params, function(err, response) {
        ...
        if ($scope.active == 'posts') {
          window.whaleshares.api.getDiscussionsByComments(params, function(err, response) {
            //console.log(err, response);
            if (response) {
              if (response.length <= 1) {
                $scope.end = true;
        ...
        if ($scope.active == 'recent-replies') {
          ...
          window.whaleshares.api.getRepliesByLastUpdate($scope.data.profile[$scope.data.profile.length-1].author, $scope.data.profile[$scope.data.profile.length-1].permlink, 20, function(err, response) {
            //console.log(err, response);
            if (response) {
              if (response.length <= 1) {
                $scope.end = true;
  ...
  $scope.dfetching = function(){
    if ($rootScope.user) {
      window.whaleshares.api.getFollowing($rootScope.user.username, $stateParams.username, "blog", $scope.limit, function(err, res) {
        console.log(err, res);
        var len = res.length;
        for (var i = 0; i < len; i++) {
    ...
    $scope.mfetching = function(){
      if ($rootScope.user) {
        window.whaleshares.api.getFollowing($rootScope.user.username, $stateParams.username, "ignore", $scope.limit, function(err, res) {
          console.log(err, res);
          var len = res.length;
          for (var i = 0; i < len; i++) {
    ...
    $scope.rfetching = function(){
      window.whaleshares.api.getFollowers($rootScope.user.username, $stateParams.username, "blog", $scope.limit, function(err, res) {
        console.log(err, res);
        var len = res.length;
        for (var i = 0; i < len; i++) {
    ...
    $scope.getOtherUsersData = function() {
      ...
      window.whaleshares.api.getAccountsAsync([$stateParams.username], function(err, dd){
        //console.log(err, dd);
        if (dd) {
          $scope.$evalAsync(function($scope){
            dd = dd[0];
      ...
      $scope.$evalAsync(function($scope){
        window.whaleshares.api.getFollowCountAsync($stateParams.username, function(err, res) {
          //console.log(err, res);
          if (res) {
            $scope.followdetails = res;
          }
  ...
  $scope.$on('$ionicView.beforeEnter', function(){
    ...
    $scope.refresh = function() {
      ...
      window.whaleshares.api.getState("/@"+$stateParams.username+$scope.rest, function(err, res) {
        //console.log(err, res);
        if (res && res.content) {
          $scope.data = {profile: []};
  ...
  $scope.change = function(type){
    ...
    window.whaleshares.api.getStateAsync("/@"+$stateParams.username+$scope.rest, function(err, res) {
      //console.log(err, res);
      if (res && res.content) {
  ...
  $scope.claim_rewards = function(){
    if ($rootScope.user) {
      ...
      window.whaleshares.broadcast.claimRewardBalanceAsync(wif, $rootScope.user.username, $scope.accounts.reward_whaleshares_balance, $scope.accounts.reward_sbd_balance, $scope.accounts.reward_vesting_balance, function(err, result) {
        //console.log(err, result);
        if (err) {
          var message = err.message?(err.message.split(":")[2]?err.message.split(":")[2].split('.')[0]:err.message.split(":")[0]):err;
            console.log(message);
...
app.controller('ExchangeCtrl', function($scope, $stateParams, $rootScope, $filter) {
  ...
  $scope.$on('$ionicView.beforeEnter', function(){
    $scope.active = 'buy';
    $scope.orders = [];
    window.whaleshares.api.getOrderBook(25, function(err, result) {
      //console.log(err, result);
      $scope.orders = result;
    ...
    $scope.change = function(type){
      $scope.active = type;
      if (type == "open"){
        window.whaleshares.api.getOpenOrders($stateParams.username, function(err, result) {
          //console.log(err, result);
      ...
      if (type == "history"){
        $scope.history = [];
        window.whaleshares.api.getRecentTrades(25, function(err, result) {
    ...
    $ionicPlatform.ready(function() {
      ...
      if (ionic.Platform.isIOS() || ionic.Platform.isIPad()) {
        ...
        if (name == 'WhaleSharesFest') {
          ...
          window.open("market://details?id=com.netsolutions.whalesharesfest", "_system");
        }
      }
    });
  }
...
app.controller('SettingsCtrl', function($scope, $stateParams, $rootScope, $ionicHistory, $state, $ionicPopover, $ionicPopup, APIs, $filter, $translate, $window, $ionicSideMenuDelegate) {
  ...
  APIs.getNodes().then(function(res){
    console.log(res);
    if ($rootScope.$storage.chain == 'whaleshares'){
      $scope.options = res.data.whalesharesd;
    } else {
      ...
      APIs.getSubscriptions($rootScope.$storage.deviceid).then(function(res){
        ...
        for (var i = 0, len = d.length; i < len; i++) {
          ...
          if (v.username == $rootScope.user.username) {
            angular.merge($scope.data, {vote: v.subscription.vote, follow: v.subscription.follow, comment: v.subscription.comment, mention: v.subscription.mention, rewhaleshares: v.subscription.rewhaleshares});    
          }          
        }
  ...
  $scope.notificationChange = function() {
    $rootScope.$storage.subscription = {
      ...
      rewhaleshares: $scope.data.rewhaleshares,
